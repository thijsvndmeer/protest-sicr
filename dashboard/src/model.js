export function simulateSICR(params, t_span, protest_days) {
  ...
  // RK4 solver
  const dt = 0.01; // smaller step size for stiffness
  const steps = Math.ceil((t_span[1] - t_span[0]) / dt);
  const sampleRate = Math.round(1.0 / dt); // sample once per day
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    // Check for NaN or blowing up, clamp to 0 and max S0
    y = y.map((v, idx) => {
        let val = v + (dt/6) * (k1[idx] + 2*k2[idx] + 2*k3[idx] + k4[idx]);
    ...
    if (i % sampleRate === 0) {
      history.push({ t: Math.round(t), S: y[0], I: y[1], C: y[2], R: y[3], N: y[1] + y[2] });
    }
  }
